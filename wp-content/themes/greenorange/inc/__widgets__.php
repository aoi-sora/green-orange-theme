...
class Custom_Content_Widget extends WP_Widget {
	...
	function Custom_Content_Widget() {
		...
		/* additional widgets */
		$widget_ops = array( 'classname' => 'widget_custom_content', 'description' => __( 'Use this widget to create custom content', 'twentyeleven' ) );
		$this->WP_Widget( 'widget_custom_content', __( 'Custom Content', 'twentyeleven' ), $widget_ops );
		$this->alt_option_name = 'widget_custom_content';		
		/* end */
	 ...
	 **/
	function widget( $args, $instance ) {
		$cache = wp_cache_get( 'widget_custom_content', 'widget' );
	...
	function flush_widget_cache() {
		wp_cache_delete( 'widget_custom_content', 'widget' );
	}
	...
	function form( $instance ) {
		$title = isset( $instance['title']) ? esc_attr( $instance['title'] ) : '';
		$content = isset( $instance['custom_content']) ? esc_attr( $instance['custom_content'] ) : '';
...
?>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'custom_content' ) ); ?>"><?php _e( 'Content:', 'twentyeleven' ); ?></label>
			<textarea class="widefat" cols="20" rows="16"   id="<?php echo esc_attr( $this->get_field_id( 'custom_content' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'custom_content' ) ); ?>" type="text"><?php echo esc_attr( $content ); ?></textarea></p>
...
/* javascript snippets */
class JS_Snippet_Widget extends WP_Widget {
	 ...
	 * @return void
	 **/
	function JS_Snippet_Widget() {
 
		/* additional widgets */
		$widget_ops = array( 'classname' => 'widget_js_snippet', 'description' => __( 'Use this widget to create custom javascript content', 'twentyeleven' ) );
		$this->WP_Widget( 'widget_js_snippet', __( 'Javascript Snippet', 'twentyeleven' ), $widget_ops );
		$this->alt_option_name = 'widget_js_snippet';		
		/* end */
	 ...
	 **/
	function widget( $args, $instance ) {
		$cache = wp_cache_get( 'widget_js_snippet', 'widget' );
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Javascript Snippet Widget', 'twentyeleven' ) : $instance['title'], $instance, $this->id_base);
		echo $instance['js_snippet'];
	 

		$cache[$args['widget_id']] = ob_get_flush();
		wp_cache_set( 'widget_js_snippet', $cache, 'widget' );
	}
	...
	function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$instance['title'] = strip_tags( $new_instance['title'] );
		$instance['js_snippet'] =   $new_instance['js_snippet'] ;
		
		$this->flush_widget_cache();

		$alloptions = wp_cache_get( 'alloptions', 'options' );
		if ( isset( $alloptions['widget_js_snippet'] ) )
			delete_option( 'widget_js_snippet' );

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete( 'widget_js_snippet', 'widget' );
	}
	...
	function form( $instance ) {
		$title = isset( $instance['title']) ? esc_attr( $instance['title'] ) : ''; 
		$content = isset( $instance['js_snippet']) ? esc_attr( $instance['js_snippet'] ) : '';
...
?>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'js_snippet' ) ); ?>"><?php _e( 'Content:', 'twentyeleven' ); ?></label>
			<textarea class="widefat" cols="20" rows="16"   id="<?php echo esc_attr( $this->get_field_id( 'js_snippet' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'js_snippet' ) ); ?>"><?php echo esc_attr( $content ); ?></textarea></p>
